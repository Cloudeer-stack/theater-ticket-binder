...
export function TicketBinder({ tickets, onSelect }: TicketBinderProps) {
  ...
  if (tickets.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-neutral-400 text-sm h-full pt-20">
        没有找到票根。点击上方 "+" 按钮添加。
      </div>
    );
  }
...
function BinderShelf({ years, onSelectYear, tickets }: BinderShelfProps) {
  return (
    <div className="flex-1 h-full bg-[#d7d3c8] overflow-y-auto p-6 relative pb-28">
      {/* Texture background */}
      ...
      <div className="text-center pt-2 pb-8 text-neutral-600 text-sm font-bold tracking-widest relative z-10 drop-shadow-sm pointer-events-none">
        我的票夹
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 relative z-10 max-w-lg mx-auto">
        {years.map((year) => {
          ...
          const THEMES = [
            ...
            'bg-rose-950',    // Burgundy
            'bg-slate-900',   // Navy
            'bg-emerald-950', // Forest Green
            'bg-amber-950',   // Brown
          ...
          return (
            ...
            >
              ...
              <div className="absolute top-1 bottom-1 left-2 right-[-8px] bg-[#f4f3f0] rounded-r-xl shadow-[inset_-2px_0_5px_rgba(0,0,0,0.1)] border-y border-r border-[#d5d1c8]">
                {/* Pages lines */}
                <div className="absolute inset-0 rounded-r-xl bg-[repeating-linear-gradient(to_bottom,transparent,transparent_3px,rgba(0,0,0,0.03)_3px,rgba(0,0,0,0.03)_4px)]" />
              </div>

              {/* Book Cover */}
              <div 
                className={`absolute inset-0 rounded-r-xl rounded-l-sm shadow-[inset_1px_1px_2px_rgba(255,255,255,0.15),4px_4px_10px_rgba(0,0,0,0.3)] flex flex-col items-center overflow-hidden transform origin-left transition-transform duration-300 group-hover:-rotate-y-6 ${colorClass}`}
              >
                {/* Book Spine Edge Overlay */}
                <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-black/60 via-black/20 to-transparent pointer-events-none z-10 shadow-[2px_0_4px_rgba(0,0,0,0.2)]" />
                
                {/* Faux Leather / Cloth Texture via SVG Noise */}
                <div 
                   className="absolute inset-0 opacity-[0.2] pointer-events-none mix-blend-overlay z-0" 
                   style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")` }} 
                />

                {/* Inner shadow for cover pillowy feel */}
                <div className="absolute inset-0 shadow-[inset_0_0_24px_rgba(0,0,0,0.6)] rounded-r-xl pointer-events-none z-0" />

                {/* Foil Stamping / Debossed Border */}
                <div className="absolute inset-3 rounded-r-lg rounded-l-sm border border-white/20 shadow-[inset_0_1px_1px_rgba(0,0,0,0.5),0_1px_0_rgba(255,255,255,0.2)] pointer-events-none z-10" />
                ...
                <div className="relative z-20 mt-10 w-[70%] bg-[#f4f3f0] shadow-[0_4px_10px_rgba(0,0,0,0.5),inset_0_0_5px_rgba(0,0,0,0.1)] rounded-sm border border-[#d5d1c8] flex flex-col items-center pt-3 pb-0 overflow-hidden transform group-hover:scale-105 transition-transform duration-300">
                    {/* Metal rivets */}
                    <div className="absolute top-1.5 left-1.5 w-1 h-1 rounded-full bg-neutral-400 shadow-[inset_0_1px_1px_rgba(0,0,0,0.5)]" />
                    <div className="absolute top-1.5 right-1.5 w-1 h-1 rounded-full bg-neutral-400 shadow-[inset_0_1px_1px_rgba(0,0,0,0.5)]" />
                    <div className="absolute bottom-1.5 left-1.5 w-1 h-1 rounded-full bg-neutral-400 shadow-[inset_0_1px_1px_rgba(0,0,0,0.5)]" />
                    <div className="absolute bottom-1.5 right-1.5 w-1 h-1 rounded-full bg-neutral-400 shadow-[inset_0_1px_1px_rgba(0,0,0,0.5)]" />

                    <span className="text-3xl font-serif font-black text-neutral-800 tracking-tighter mt-1" style={{ textShadow: '0 1px 0 rgba(255,255,255,0.8)' }}>
                        {year}
                    </span>
                    
                    <div className="w-full bg-neutral-800/5 mt-3 py-1.5 border-t border-black/10 flex justify-center">
                       <div className="text-[8px] text-neutral-600 font-bold uppercase tracking-[0.2em] text-center w-full">
...
function BinderView({ tickets, onSelect, onBack, year }: BinderViewProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#d7d3c8] relative overflow-hidden">
      {/* Texture background */}
      <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'radial-gradient(#000 1px, transparent 1px)', backgroundSize: '12px 12px' }}></div>
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex items-center justify-between z-50">
        <button 
          onClick={onBack}
          className="w-10 h-10 flex items-center justify-center rounded-full bg-white/50 backdrop-blur-md shadow-sm border border-white hover:bg-white text-neutral-700 transition"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="text-neutral-600 text-sm font-bold tracking-widest bg-white/50 backdrop-blur-md px-4 py-2 rounded-full border border-white">
            ...
            <div className="absolute left-[-16px] top-10 bottom-10 w-8 z-30 flex flex-col justify-evenly py-6 pointer-events-none transform translate-z-[1px]">
                {[...Array(4)].map((_, i) => (
                    <div key={i} className="flex flex-row items-center w-full">
                        <div className="w-4 h-6 border-2 border-r-0 border-neutral-400 bg-neutral-300 rounded-l-full shadow-md" />
                        <div className="w-10 h-1 bg-gradient-to-r from-neutral-400 to-transparent opacity-30 -ml-1" />
                    </div>
                ))}
            </div>
                   ...
                   >
                     ...
                     >
                         ...
                         {ticket.ticketImage ? (
                             ...
                             <div className="absolute inset-0 z-0 pointer-events-none">
                                <div 
                                    className="absolute top-1/2 left-1/2 flex flex-col items-center justify-center p-4 pt-10 text-neutral-400"
                                   style={{ width: '200%', height: '50%', transform: 'translate(-50%, -50%) rotate(-90deg)' }}
                                >
                         ...
                         <div className="absolute bottom-4 right-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-md text-xs font-bold text-neutral-600 shadow shadow-black/10 border border-white/50 z-30 pointer-events-none">
